...
x_train, y_train, treatment_train = df_train[features], df_train['label'], df_train['is_treat']
x_test, y_test, treatment_test = df_test[features], df_test['label'], df_test['is_treat']

denominators = []
for fea in features:
    denominators.append(df_de[fea].max())
...
valid_data_size = x_test.shape[0]
Uplift = UpliftModel(embedding_dim=embedding_dim, seq_len=seq_len, batch_size=batch_size,
                     ...
                     train=[x_train, y_train, treatment_train], test=[x_test, y_test, treatment_test])
uplift_model = Uplift.model_train()

uplift_model.load_weights('./epoch_lazada.h5')
